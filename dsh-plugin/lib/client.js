...
${Du(u)}`),e("已下载 Prompt + 批注全文")}function M0(u){let e,n;const t=(r,o=2200)=>{(!n||!n.isConnected)&&(n=document.createElement("div"),n.className="fxr-toast",u().appendChild(n)),n.textContent=r,n.classList.add("fxr-toast-visible"),e&&window.clearTimeout(e),e=window.setTimeout(()=>{n?.classList.remove("fxr-toast-visible")},o)};return t.dispose=()=>{e&&window.clearTimeout(e),n?.remove(),n=void 0},t}const dn={insertion:{label:"插入",icon:"➕",title:"插入新文字",key:"I"},deletion:{label:"删除",icon:"✂️",title:"标记为删除",key:"D"},substitution:{label:"替换",icon:"🔁",title:"替换为新文字",key:"S"},highlight:{label:"高亮",icon:"🔆",title:"高亮关注",key:"H"},comment:{label:"评论",icon:"💬",title:"添加评论",key:"C"}},I0=["deletion","substitution","highlight","comment","insertion"],hn=262,R0=40,X=8;function B0(u){const e=document.createElement("div");e.className="fxr-sel-menu";for(const n of I0){const t=dn[n],r=document.createElement("button");r.className=`fxr-sel-menu-btn fxr-sel-menu-${n}`,r.title=`${t.title}（快捷键 ${t.key}）`,r.innerHTML=`<span class="fxr-btn-icon">${t.icon}</span><span class="fxr-btn-label">${t.label}</span>`,r.addEventListener("pointerdown",o=>{o.preventDefault()}),r.addEventListener("click",()=>{u.annotate(n)}),e.appendChild(r)}return u.root.appendChild(e),{el:e,show(n){if(!u.store.state.source)return;const t=u.root.getBoundingClientRect();e.style.display="flex";let r=n.top-t.top-X-R0;r<X&&(r=n.bottom-t.top+X);let o=n.left-t.left+n.width/2-hn/2;o=Math.max(X,Math.min(o,t.width-hn-X)),e.style.top=`${Math.max(X,r)}px`,e.style.left=`${o}px`},hide(){e.style.display="none"}}}function L0(u){switch(u.type){case"insertion":return"插入";case"deletion":return"删除";case"substitution":return"替换";case"highlight":return"高亮";case"comment":return"评论"}}function q0(u){const e=document.createElement("aside");e.className="fxr-comments-panel";const n=document.createElement("div");n.className="fxr-drawer-backdrop",e.innerHTML=`
    ...
    <div class="fxr-comments-list"></div>`;const t=e.querySelector(".fxr-comments-list"),r=e.querySelector(".fxr-comments-count"),o=e.querySelector('[data-act="undo"]'),i=e.querySelector('[data-act="redo"]'),c=e.querySelector('[data-act="clear"]'),a=()=>{n.classList.remove("show"),e.classList.remove("open")},s=()=>{n.classList.add("show"),e.classList.add("open")};o.addEventListener("click",()=>{u.undo()||u.notify("没有可撤销的操作")}),i.addEventListener("click",()=>{u.redo()||u.notify("没有可重做的操作")}),c.addEventListener("click",()=>{u.store.state.annotations.length!==0&&window.confirm("确定清除全部批注？")&&u.clearAnnotations()}),e.querySelector('[data-act="submit"]')?.addEventListener("click",()=>u.submit()),e.querySelector('[data-act="close"]').addEventListener("click",a),n.addEventListener("click",a),P0(u,e);function f(p){const l=document.createElement("div");l.className=`fxr-comment-entry fxr-comment-entry-${p.type}`,l.dataset.id=p.id;const d=L0(p),g=p.quotedText||(p.type==="insertion"?p.insertedText:"")||"",m=p.type==="insertion"||p.type==="substitution";l.innerHTML=`
      ...
      <div class="fxr-comment-extra"></div>`,l.querySelector(".fxr-comment-quote").textContent=g;const _=l.querySelector(".fxr-comment-extra");if(p.type==="substitution")_.innerHTML='<span class="fxr-sub-old"></span><span class="fxr-sub-arrow"> → </span><span class="fxr-sub-new"></span>',_.querySelector(".fxr-sub-old").textContent=p.quotedText,_.querySelector(".fxr-sub-new").textContent=p.replacement??"";else if(p.type==="insertion")_.innerHTML='<span class="fxr-ins-text"></span>',_.querySelector(".fxr-ins-text").textContent=p.insertedText??"";else if(p.type==="comment"){const b=document.createElement("textarea");b.className="fxr-comment-note",b.placeholder="输入审阅意见…",b.value=p.comment??"",b.addEventListener("input",()=>{u.store.setComment(p.id,b.value),u.onSilentMutation()}),_.appendChild(b)}l.querySelector(".fxr-comment-locate").addEventListener("click",()=>{u.flash(p)});const x=l.querySelector(".fxr-comment-edit");return x&&x.addEventListener("click",async()=>{const b=p.type==="insertion",k=await Yu({title:b?"修改插入文字":"修改替换后的新文字",value:(b?p.insertedText:p.replacement)??"",anchor:l.getBoundingClientRect(),allowEmpty:p.type==="substitution"});k!=null&&u.store.updateAnnotation(p.id,b?{insertedText:k}:{replacement:k})}),l.querySelector(".fxr-comment-del").addEventListener("click",()=>{u.store.removeAnnotation(p.id)}),l}function h(){const p=u.store.state.annotations;r.textContent=String(p.length),o.disabled=!u.store.canUndo,i.disabled=!u.store.canRedo;const l=document.activeElement,d=l instanceof Element?l.closest(".fxr-comment-entry"):null,g=d?.dataset.id??null;if(p.length===0){t.innerHTML='<div class="fxr-comments-empty">尚无批注。选中正文文字后点击批注按钮，或在选区旁的浮动菜单操作。</div>';return}const m=[...p].sort((x,b)=>x.srcStart-b.srcStart),_=g!==null&&m.some(x=>x.id===g);for(const x of[...t.children])_&&x===d||x.remove();for(const x of m)_&&x.id===g&&d!==null?t.appendChild(d):t.appendChild(f(x));_&&l instanceof HTMLElement&&l.isConnected&&l.focus({preventScroll:!0})}return{el:e,backdrop:n,refresh:h,open:s,close:a,toggle(){e.classList.contains("open")?a():s()}}}function P0(u,e){const n=document.createElement("div");n.className="fxr-resizer",n.title="拖动调整批注栏宽度",e.appendChild(n);let t=!1;const r=240,o=640,i=f=>{u.root.style.setProperty("--fxr-panel-w",`${Math.round(f)}px`)},c=f=>{f.button===0&&(t=!0,n.setPointerCapture(f.pointerId),f.preventDefault())},a=f=>{if(!t)return;const h=e.parentElement;if(!h)return;const p=h.getBoundingClientRect(),l=Math.min(o,Math.max(r,p.right-f.clientX-1));i(l)},s=()=>{if(t){t=!1;try{const f=u.root.style.getPropertyValue("--fxr-panel-w");f&&window.localStorage.setItem("fx-review:panelWidth",f)}catch{}}};n.addEventListener("pointerdown",c),n.addEventListener("pointermove",a),n.addEventListener("pointerup",s),n.addEventListener("pointercancel",s),n.addEventListener("dblclick",()=>{u.root.style.removeProperty("--fxr-panel-w");try{window.localStorage.removeItem("fx-review:panelWidth")}catch{}})}const $0=["insertion","deletion","substitution","highlight","comment"];function I(u,e,n,t){const r=document.createElement("button");return r.type="button",r.className=u,r.title=e,r.innerHTML=n,r.addEventListener("click",t),r}function O0(u){const e=document.createElement("header");e.className="fxr-toolbar";let n=null;if(u.variant==="full"){const l=document.createElement("div");l.className="fxr-brand",l.innerHTML='<span class="fxr-brand-name">fx-review</span><span class="fxr-brand-sub"></span>',n=l.querySelector(".fxr-brand-sub"),e.appendChild(l)}const t=document.createElement("div");t.className="fxr-btn-group";for(const l of $0){const d=dn[l];t.appendChild(I(`fxr-btn fxr-btn-anno fxr-btn-anno-${l}`,`${d.title}（选中后按 ${d.key}）`,`<span class="fxr-btn-icon">${d.icon}</span><span class="fxr-btn-label">${d.label}</span>`,()=>u.annotate(l)))}e.appendChild(t);const r=document.createElement("div");r.className="fxr-btn-group";const o=I("fxr-btn fxr-btn-ghost","撤销（⌘/Ctrl+Z）","↩",()=>{u.undo()||u.notify("没有可撤销的操作")}),i=I("fxr-btn fxr-btn-ghost","重做（⌘/Ctrl+Shift+Z）","↪",()=>{u.redo()||u.notify("没有可重做的操作")}),c=I("fxr-btn fxr-btn-ghost","清除全部批注","🗑 清空",()=>{u.store.state.annotations.length!==0&&window.confirm("确定清除全部批注？")&&u.clearAnnotations()});r.append(o,i,c),e.appendChild(r);const a=document.createElement("div");a.className="fxr-btn-group",a.appendChild(I("fxr-btn fxr-btn-export","复制带 CriticMarkup 批注的全文","复制批注",()=>void u.copy(!1))),u.variant==="full"&&a.appendChild(I("fxr-btn fxr-btn-export","下载带批注的全文 .md","下载批注",()=>u.download(!1))),a.appendChild(I("fxr-btn fxr-btn-export","复制引导 Prompt + 批注全文（⌘/Ctrl+Shift+C）",u.variant==="full"?"复制(含Prompt)":"复制含P",()=>void u.copy(!0))),u.variant==="full"&&a.appendChild(I("fxr-btn fxr-btn-export","下载 Prompt + 批注全文 .md","下载(含Prompt)",()=>u.download(!0))),e.appendChild(a);const s=document.createElement("div");s.className="fxr-btn-group fxr-toolbar-tail",s.appendChild(I("fxr-btn fxr-btn-ghost","编辑「含 Prompt」导出的引导词","Prompt",()=>{u.openPromptEditor()}));let f=null;u.variant==="full"&&(f=I("fxr-btn fxr-btn-ghost","切换主题","🌙",()=>{u.setTheme(u.getTheme()==="dark"?"light":"dark")}),s.appendChild(f));const h=I("fxr-btn fxr-btn-ghost fxr-panel-toggle","打开评论与批注栏",'📋 <span class="fxr-panel-toggle-count">0</span>',()=>u.togglePanel());s.appendChild(h);const p=h.querySelector(".fxr-panel-toggle-count");return e.appendChild(s),{el:e,setFileName(l){n&&(n.textContent=l||"未打开文件"),e.title=l},setCount(l){p.textContent=String(l)},setUndoRedo(l,d){o.disabled=!l,i.disabled=!d},setThemeIcon(l){f&&(f.textContent=l==="dark"?"☀️":"🌙")}}}function H0(u){const e=document.createElement("div");e.className="fxr-modal-overlay";const n=document.createElement("div");n.className="fxr-modal-box",n.innerHTML=`
    <h3>编辑引导 Prompt</h3>
